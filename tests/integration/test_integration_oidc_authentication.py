...
from tests.integration.integ_utils \
    import create_client, ConjurUser, AuthenticationStrategyType
...
class TestOidcAuthentication(AsyncTestCase):
    ...
    async def test_oidc_authentication_success(self):
        c = await create_client(
                                self.valid_user.id,
                                self.valid_user.secret,
                                AuthenticationStrategyType.OIDC,
    ...
    async def _add_test_data(cls):
        ...
        await c.set(
                'conjur/authn-oidc/test-service/provider-uri',
                'https://oidc-server'
            )
        await c.set(
                'conjur/authn-oidc/test-service/id-token-user-property',
                'sub'
            )
        ...
        x = requests.post(
                ...
                verify=True,
                auth=HTTPBasicAuth(
                                    'client-credentials-mock-client',
                                    'client-credentials-mock-client-secret'
                                )
                        )

        cls.valid_user: ConjurUser = ConjurUser(
                user_id='john.williams',
                secret=x.json()['access_token']
            )
